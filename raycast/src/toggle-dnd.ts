...
export default async function Command() {
  const toast = await showToast({ style: Toast.Style.Animated, title: "Toggling Do Not Disturb…" });
  try {
    execSync(`shortcuts run "DND Until I Leave" 2>&1 || true`, { encoding: "utf-8", timeout: 10_000 });
    toast.hide();
